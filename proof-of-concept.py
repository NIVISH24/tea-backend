...
def generate_subheadings_and_related_topics(topic_name):
    prompt = f"For the topic '{topic_name}', list all the types, subheadings, and related topics necessary to fully understand this course. Provide the output as a 1-Dimensional python list - between a [] Eg: [x, y, z] and nothing else."
    output = ollama.generate(model="llama3.2", prompt=prompt)
    print(output["response"])
